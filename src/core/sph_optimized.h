#ifndef SPH_OPTIMIZED_H
#define SPH_OPTIMIZED_H
...
namespace test { class SPHOptimizedTestAdapter; }


class SPHOptimized : public FluidParticleSystem
{
  public:
    SPHOptimized(const boost::compute::context & ctx, SPHParams *params)
      ...
      , m_prev_velocity_buf()
    {
      ...
      if (!init())
      {
        throw std::runtime_error("Failed to construct SPHOptimized: OpenCL initialization failed");
      }
    }
  ...
  private:
    ...
    friend class test::SPHOptimizedTestAdapter;
};

#endif // SPH_OPTIMIZED_H
